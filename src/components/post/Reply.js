...
import {triggleCommentLike,deleteReplyDoc}from "../../services/firebase"
import {PROFILE} from '../../constants/Routes'
import useDateCreated from "../../hooks/useDateCreated"
import DeleteComment from "./deleteComment"

function Reply({username,activeUserUsername,commentDocId,text,likes,docId,activeUserUserId,didLike,
    inputRef,dateCreated,dispatcher,setRepliesCount}){
    //console.log('reply')
    ...
    function replyHandler(){
        ...
        dispatcher({type:'commentType', value:{isReply:true,replyDocId:commentDocId}});
        inputRef.current.focus();
    }
    const createdText = useDateCreated(dateCreated);
    async function deleteComment(){
        await deleteReplyDoc(docId,commentDocId);
        setRepliesCount(prv=>prv-1);
